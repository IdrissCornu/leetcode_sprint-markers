...
function main() {
  ...
  const map = readSprintMapFromFile(mapName);

  console.time("Execution time")
  const mostVisitedMarker = getMostVisitedMarker(map);
  console.timeEnd("Execution time")

  console.log("Most visited marker: " + mostVisitedMarker);
}

const fs = require("node:fs");

function readSprintMapFromFile(mapName) {
  const data = fs.readFileSync('./maps/map-' + mapName + '.json');
